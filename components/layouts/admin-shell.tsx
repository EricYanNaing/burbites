    ...
    <section className="min-h-screen bg-[#f4eee8] text-secondary">
      ...
      >
        <button
          ...
          className={`absolute inset-0 bg-[#1e1214]/48 transition ${
            menuOpen ? "opacity-100" : "opacity-0"
          }`}
        />

        <aside
          className={`absolute left-0 top-0 h-full w-[86vw] max-w-[320px] border-r border-white/8 bg-[#26191a] text-white shadow-[0_24px_60px_rgba(0,0,0,0.32)] transition-transform duration-300 ${
        ...
        <div className="rounded-[24px] border border-white/8 bg-white/6 p-4">
          ...
          <form action={signOutAction} className="mt-4">
            <button
              type="submit"
              onClick={onNavigate}
              className={`inline-flex w-full items-center justify-between rounded-[18px] border border-white/10 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/8 ${
                mobile ? "bg-white/8" : "bg-black/18"
              }`}
            >
              Sign out
...
function MenuItem({
  ...
  const content = (
    <div
      className={`group flex items-center gap-3 rounded-[22px] border px-3 py-3 transition ${
        active
    ...
    >
      <div
        className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${
          active ? "bg-primary text-white" : "bg-white/8 text-white"
        }`}
      >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p
          className={`mt-1 text-xs leading-5 ${
            active ? "text-secondary/60" : "text-white/52"
          }`}
        >
          {item.description}
        </p>
      ...
      <ChevronRight
        className={`h-4 w-4 shrink-0 ${
          active ? "text-secondary/40" : "text-white/32"
        }`}
      />
    </div>
  );
